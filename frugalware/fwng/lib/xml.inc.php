...
if (class_exists('XMLParser') != True)
{
    class XMLParser
    {
	/**
	...
	/**
	 * Initiates and runs PHP XML parser
	 */
	...
	private function StartElement($parser, $name, $attrs = array())
	{
		...
		//If it is not root level, use the stack to find the parent
		else
		{
...
if (class_exists('XMLTag') != True)
{
	 ...
	 *
	 * So, if the tag does not contain child tags, and just contains a string, it would go here
	 *
	 * @var stat
	 */
	public $tagData;
	...
	public function AddChild($name, $attrs, $parents)
	{
		...
		//array was not properly created, output a more informative error than
		//PHP otherwise would.
		...
		//Add the reference of it to the end of an array member named for the tag name
		$this->{$name}[] = &$child;
