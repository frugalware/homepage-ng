...
{
    try {
        global $sqlhost, $sqluser, $sqlpass, $fwng_root;
        if ( isset($_GET['id']) and is_numeric($_GET['id']) )
        {
            if (isset($_GET['s'])) {
                ...
                else if (isset($_GET['op']) && !strlen($_GET['op']) && ($_GET['op'] != "groups"))
                    pkg_from_id($_GET['id']);
            }
            else {
                pkg_from_id($_GET['id']);
        ...
        else
        {
            ...
            $content = "<form class=\"packages\" name=\"pkgsrch\" action=\"" . $fwng_root . "packages\" method=\"get\">
            <h2><img src=\"" . $fwng_root . "images/icons/packages.png\" />".gettext("Package search")."</h2>
            <div class=\"emptybox\">
            <table class=\"package\">
                <tr>
                ...
                <tr>
                    <td style=\"vertical-align: middle;\"><b>".gettext("Type")."</b></td>
                    <td style=\"vertical-align: middle;\">
                        <input type=\"radio\" name=\"op\" value=\"pkg\" " . (!in_array(checkValue('op'), array("file", "groups")) ? "checked=\"checked\" " : "") . "/>".gettext("packages")."
                        <input type=\"radio\" name=\"op\" value=\"file\" " . (checkValue('op') == "file" ? "checked=\"checked\" " : "") . "/>".gettext("files")."
                        <input type=\"radio\" name=\"op\" value=\"groups\" " . (checkValue('op') == "groups" ? "checked=\"checked\" " : "") . "/>".gettext("groups")."
                    </td>
                </tr>
                <tr>
    ...
    function addEngine()
    {
        if ((typeof window.sidebar == \"object\") && (typeof window.sidebar.addSearchEngine == \"function\"))
        {
            ...
            \"http://frugalware.org/static/package/Frugalware_Packages.src\",
            \"http://frugalware.org/static/package/Frugalware_Packages.png\",
    ...
    </script><p>" . gettext("Click <a href=\"javascript:addEngine()\">here</a> to install the Firefox search plugin.") . "</p></div>\n";

            print $content;
        }
    }
    catch (Exception $e) {
        print "Can't access to database";
    }
}
...
function search_pkg()
{
    ...
    $sub = (checkValue('sub') == "on") ? 1 : 0; # whether the search is for a substring or exact match
    ...
    (checkValue('desc') == "on" || checkValue('desc') == 1) ? $query .= "(pkgname rlike '$search' or `desc` rlike '$search')" : $query .= "(pkgname rlike '$search')";
